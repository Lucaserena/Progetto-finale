...
if (isset($_POST["txtcomment"])&& isset($_POST["idbook"]) && (preg_match ("/</", $_POST["txtcomment"])== false)) {
    $account = $_SESSION["username"];
    $text = $_POST["txtcomment"];
    $user = $_SESSION["username"];
    $libro = $_POST["idbook"];
    $db = new PDO("mysql:dbname=databaselibreria; host=localhost", "root", "");
	$insert = $db->prepare("UPDATE loans SET review = :t WHERE username = '$user' AND idBook = '$libro' AND returned = 1");
    $insert-> bindParam (':t', $text);  //prevent SQL injection with parameters
    $insert-> execute();
...
if (isset($_SESSION["username"]) && isset($_POST["returnedBook"])) {
    ...
    <form method="POST" action="review.php">
        <textarea id="txtcomment" name="txtcomment" rows= "5" cols= "40" required></textarea><br />
